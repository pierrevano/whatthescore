...
const getLeadingScore = async (firstRes, secondRes) => {
  try {
    ...
    if (firstRes === secondRes) {
      scoreGrid = `<span class="match-score-number match-score-number">${firstRes}</span><span class="match-score-divider">:</span><span class="match-score-number">${secondRes}</span>`;
    } else if (leadingScore == firstRes) {
